...
Completion behaviour can be controlled with `bbdb-completion-type'."
  ...
  (let* ((end (point))
    ...
    (message "orig :%s" orig)
    ;; exact-match only is t, when you enter a full net, such as aaa@bbb.com
    ;; and it is found in the db
    (message "exact-match : %s" exact-match)
    (cond
     ...
     ((or (null completion)
      ...
      (message "entering 1")
      (message "null completion? %s" (null completion))
